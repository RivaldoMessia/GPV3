...
def is_suitable_for_planting(forecast, crop):
    ...
    return (requirements["min_temp"] <= temp <= requirements["max_temp"] and
            requirements["min_rainfall"] <= rainfall * 8 <= requirements["max_rainfall"])
...
# --- SIDEBAR ---
st.sidebar.header("🔍 Enter Environmental Conditions")
rain = st.sidebar.slider("Rainfall (mm)", 300, 1300, 800, key="rain_slider")
fert = st.sidebar.slider("Fertilizer", 40, 100, 70, key="fert_slider")
temp = st.sidebar.slider("Temperature (°C)", 20, 40, 30, key="temp_slider")
n = st.sidebar.slider("Nitrogen (N)", 60, 90, 75, key="n_slider")
p = st.sidebar.slider("Phosphorus (P)", 15, 30, 20, key="p_slider")
k = st.sidebar.slider("Potassium (K)", 15, 30, 20, key="k_slider")

st.sidebar.header("🌱 Planting Scheduler")
city = st.sidebar.text_input("City (e.g., Nairobi, Iowa)", "Nairobi", key="city_input")
crop = st.sidebar.selectbox("Select Crop", list(CROP_REQUIREMENTS.keys()), key="crop_select")
api_key = st.sidebar.text_input("OpenWeatherMap API Key", type="password", key="api_key_input")
...
if st.button("Get Planting Schedule"):
    if not api_key:
        ...
        if weather_data:
            ...
            suitable_dates = sorted(set(suitable_dates))
